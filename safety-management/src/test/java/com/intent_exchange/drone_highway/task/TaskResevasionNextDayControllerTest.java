...
/** TaskResevasionNextDayController のテストクラス */

// TestConfigとModelMapperConfigのModelMapperインスタンス作成のbeenが重複してエラーのため、Bean 定義の上書きを許可します。
@ExtendWith(MockitoExtension.class)
class TaskResevasionNextDayControllerTest {

  @InjectMocks
  private TaskResevasionNextDayController controller;
  ...
  void testPerformTask() {
    ...
    verify(service).conformityAssessment(eq(dto),
        eq(AirwayDesignAreaInfoResevasionNextDayLogic.class),
        eq(AirwayDesignAreaInfoResevationProcessorLogic.class));
  }
}
